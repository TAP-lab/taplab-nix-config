...
6. To use the install script, simply run the following command:
    ...
    ```
7. The script automatically installs NixOS and the configuration. It will automatically reboot when finished.
...
- Most things should be set up after the installation, wifi can be set up by running `wifi-setup` in terminal to pull the credentials from the local server (provided ethernet is connected), or by setting it up manually just like you would on windows.
...
- The system has 4 network shares automatically mounted, manuhiri, Hacklings, Inventors Guild, and mema. The first 3 should mount automatically provided the laptop is on the TAP-lab network. The mema share requires credentials to access, which can be pulled from the local server by running the `mema-setup` command in terminal.
...
- OrcaSlicer can be pre-configured with the TAP-lab 3D printer profiles by running the `orcaslicer-setup` command in terminal. This also requires the laptop to be on the TAP-lab network.

- All of the above can be run in one go with the `credentials-setup` command.
...
The `wifi-setup`, `mema-setup`, `edge-setup`, `minecraft-setup`, and `orcaslicer-setup` commands (in `scripts/credentials/`) all pull their respective credentials/config from a local "credserver" on the TAP-lab network. Each script:

1. Looks up which credserver to use, either from `/etc/servers.ini` (pinging each configured server in order until one responds), or via an explicit `--server <name>` / `--ip <address>` flag.
...
7. To use the install script, simply run the following command:
    ...
    - `--output` can be used to specify the output to use for the nix flake. This is currently only `nixos` and should not be changed.
    ...
    - `--skip-disko` can be used to skip the disk partitioning step.
...
- `--output` can be used to specify the output to use for the nix flake. The script will automatically use the current hostname, so this is only necessary if you want to use a different flake output.
- `-g` will perform garbage collection after rebuild to free up disk space.
- All of these flags can be used in short form, by prefixing the first letter with a single dash, for example `-a` for `--action`.
- The valid single-letter flags are `-a`, `-b`, `-o`, and `-g`.
